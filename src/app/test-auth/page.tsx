...
export default function TestAuthPage() {
  ...
  const getRoleIcon = (role: string) => {
    switch (role) {
      ...
      case 'seller': return <Shield className="h-4 w-4 text-blue-500" />
      case 'buyer': return <User className="h-4 w-4 text-green-500" />
              ...
              <div className="p-4 bg-green-50 rounded-lg">
                ...
                <div className="mt-4">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div className="flex items-center gap-2">
                      <span className={user.permissions.canSell ? "text-green-600" : "text-red-600"}>
                        {user.permissions.canSell ? "✅" : "❌"}
                      </span>
                      Peut vendre
                    </div>
                    <div className="flex items-center gap-2">
                      <span className={user.permissions.canBuy ? "text-green-600" : "text-red-600"}>
                        {user.permissions.canBuy ? "✅" : "❌"}
                      </span>
                      Peut acheter
                    </div>
                    <div className="flex items-center gap-2">
                      <span className={user.permissions.canRent ? "text-green-600" : "text-red-600"}>
                        {user.permissions.canRent ? "✅" : "❌"}
                      </span>
                      Peut louer
                    </div>
                    <div className="flex items-center gap-2">
                      <span className={user.permissions.canSupervise ? "text-green-600" : "text-red-600"}>
