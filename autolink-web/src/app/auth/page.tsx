...
export default function AuthPage() {
  ...
  const buildSocialRedirectUri = () => {
    if (typeof window === "undefined") {
      return process.env.NEXT_PUBLIC_SOCIAL_REDIRECT_URI || "http://localhost:3000/auth/social/callback"
    }
    return process.env.NEXT_PUBLIC_SOCIAL_REDIRECT_URI || `${window.location.origin}/auth/social/callback`
  }
  
  const startSocialLogin = (provider: "google" | "facebook") => {
    const baseUrl = process.env.NEXT_PUBLIC_KEYCLOAK_URL
    const realm = process.env.NEXT_PUBLIC_KEYCLOAK_REALM || "autolink"
    const clientId = process.env.NEXT_PUBLIC_KEYCLOAK_CLIENT_ID || "autolink-api"
    if (!baseUrl) {
      setError("La configuration Keycloak est manquante. Vérifiez NEXT_PUBLIC_KEYCLOAK_URL.")
      return
    }
    
    const redirectUri = buildSocialRedirectUri()
    const state = crypto.randomUUID ? crypto.randomUUID() : Math.random().toString(36).substring(2)
    if (typeof window !== "undefined") {
      localStorage.setItem("autolink_social_state", state)
    }
    ...
    window.location.href = `${baseUrl}/realms/${realm}/protocol/openid-connect/auth?${params.toString()}`
  }
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Button variant="outline" className="w-full" type="button" onClick={() => startSocialLogin("google")}>
              ...
              <Button variant="outline" className="w-full" type="button" onClick={() => startSocialLogin("facebook")}>
